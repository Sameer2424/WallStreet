...
from accounts.models import User, UserProfile, PotentialUser, UserFollowing
...
from .mixins import MailMixin, activate_user_from_activation_link
...
class LoginWithoutPasswordView(TokenObtainPairView):
    permission_classes = [AllowAny]
...
class ActivateUserAPIView(APIView):
    ...
    def post(self, request):
        try:
            ...
            user = activate_user_from_activation_link(uid, token)
...
class LoggedInUserProfileAPIView(APIView):
    def get(self, request):
        ...
        return Response(
            {"profile_id": profile.id, "profile_runs": profile.runs,
...
class GetUserFollowers(APIView):
    def get(self, request, *args, **kwargs):
        ...
        if profile_id:
            ...
            followers = UserProfileSerializer(userprofile.followers.all(), many=True, read_only=True)
...
class GetUserFollowing(APIView):
    def get(self, request):
        profile_id = self.request.query_params.get("profile_id")
        if profile_id:
            following_ids = UserFollowing.objects.filter(user_id=profile_id).values_list(
                'following_user', flat=True
            )
            followings = UserProfile.objects.filter(id__in=following_ids)
            profiles_following = UserProfileSerializer(followings, many=True, read_only=True)
            return Response(profiles_following.data)
        return Response([])
...
class PitchReadOnlyViewSet(ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        pitches = Pitch.objects.select_related("userprofile").filter(can_be_shown=True)
        if self.request.query_params.get("filter") == "user":
            ...
            followers = [p.id for p in profile.followers.all()]
            return (
                pitches.exclude(userprofile_id=profile.id)
                .filter(userprofile__followers__id__in=followers)
                .order_by("-id")
            )
...
class PitchViewSet(ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        data = request.data
        data_keys = list(data.dict().keys())
        data_dict = [json.loads(key) for key in data_keys]
        new_data = {}
        for dd in data_dict:
            for key in dd:
                new_data[key] = dd[key]
        serializer = PitchSerializer(
            instance=self.get_object(), data=new_data, partial=True
        )
        serializer.is_valid(raise_exception=True)
        instance = serializer.instance
        for field, value in serializer.validated_data.items():
            setattr(instance, field, value)
        instance.save()
        return Response(data={"pitch": instance.id}, status=status.HTTP_202_ACCEPTED)
...
class FriendsSuggestionAPIView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        my_friends = me.followers.all()
        following_ids = UserFollowing.objects.filter(user_id=me.id).values_list(
            'following_user', flat=True
        )
        following_ids = list(following_ids) + [me.id]
        if my_friends.exists():
            friends_of_my_friends = UserProfile.objects.none()
            for each_friend in my_friends:
                friends_of_my_friends |= each_friend.followers.all()
            profiles = friends_of_my_friends.exclude(id__in=following_ids).distinct()
            try:
                return random.sample(list(profiles), 20)
            except ValueError:
                return profiles
        profiles = UserProfile.objects.exclude(id__in=following_ids).filter().distinct()
        try:
            return random.sample(list(profiles), 20)
        except ValueError:
            return profiles
