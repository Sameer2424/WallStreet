    ...
    def send_email_for_account_activation(self, user):
        ...
        message = render_to_string(
            "accounts/champhunt_account_activation.html",
             {"user": user, 
             "authentication_link": "http://champhunt.com/activate/" + uid + "/" + token + "/"
             },
        )
        to_email = user.email
...
class PitchSerializer(FilterAdultContent, serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        response["author"] = {
            "name": f"{instance.userprofile.first_name} {instance.userprofile.last_name}",
            "url": "",
            "avatar": instance.userprofile.profile_pic.name
        }
        response["post"] = {
            'post_id': instance.id,
